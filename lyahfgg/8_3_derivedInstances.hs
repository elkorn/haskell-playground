...
main :: IO ()
main = do
  mikeD <- return $ Person {firstName = "Michael", lastName="Diamond", age=43}
  adRock <- return $ Person {firstName = "Adam", lastName="Horovitz", age=41}
  print $ mikeD == mikeD
  print $ adRock == mikeD
  print $ mikeD `elem` [adRock, mikeD]
  print $ adRock 
  mikeD' <- return $ read "Person {firstName = \"Michael\", lastName=\"Diamond\", age=43}"
  print $ mikeD == mikeD'
  print $ T `compare` F
