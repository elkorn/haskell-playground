module LevelGen.DefaultMonadic where
...
generateLevel :: LevelSpec -> StateT Level IO ()
generateLevel spec = do
    baseLevel <- get
    rooms <- liftIO $ generateRooms baseLevel [] 4
    liftIO $ print $ map roomCoordinates rooms
    -- print $ map (getCenter . roomCoordinates) rooms
    corridors <- liftIO $ generateCorridors rooms
